/* This file was generated from TypeScript source C:/Users/autobuild/turbulenz/engine/tslib/webgl/physicsdevice.ts */

// TODO: the sub class should inherit from this interface
...
var WebGLPhysicsConfig = {
    ...
    CONTACT_MAX_Y_SEPERATION: // different coeffecient for static-dynamic case.
    //
    // (Contact persistance)
    // Amount of seperation permitted before contact cache is destroyed
    0.05,
    CONTACT_MAX_SQ_XZ_SEPERATION: // Amount of squared tangential seperation permitted before contact cache is destroyed
    2 * (0.245 * 0.245),
    CONTACT_INHERIT_SQ_SEPERATION: // Amount of seperation permitted for a new contact to inherit existing cache's impulse cache
    3 * (0.75 * 0.75),
    CONTACT_EQUAL_SQ_SEPERATION: //always inherit closest
    ...
    0.35,
    CONTINUOUS_ANGULAR_SQ: // Threshold for square of angular velocity to be considered moving fast enough to be collided
    // continuously against static/sleeping objects.
    // this is multiplied with time step.
    0.25,
    CONTINUOUS_LINEAR_BULLET: // Threshold for ratio of squared linear speed to radius for object to be considered moving fast enough
    // to be collided continuously against other dynamic objects.
    // This is a 'per-step' ratio.
    0.75,
    CONTINUOUS_ANGULAR_BULLET: // Threshold for squared angular speed to be considered for continuous collisions against other dynamics.
    // This is a 'per-step' value.
    0.5,
    CONTINUOUS_SLOP: // Amount of extra slop permitted in continuous collisions.
    ...
    0.01,
    SLEEP_ANGULAR_SQ: // squared angular velocity to be considered 'at rest'.
    ...
    0.1,
    SLEEP_DELAY: // number of world updates body must be 'at rest' to be allowed to sleep.
    60,
    MAX_ANGULAR: //
    // (Misc)
    Math.PI,
    QUADRATIC_THRESHOLD: //maximum angular velocity per time-step before clamping in integration occurs.
    //
    // (General)
    1e-8,
...
WebGLPhysicsPrivateTriangleArray.prototype = {
    version: 1,
    TRIANGLE_SIZE: // Size of each 'triangle' in triangles array.
    17,
...
WebGLPhysicsPrivateBody.prototype = {
    version: 1,
    computeDeltaVelocity: // Used for kinematics.
    // TODO: Should be used for convexSweep to permit non-linear sweeps.
    function collisionObjectComputeDeltaVelocityFn(timeStep, from, to, inputVelocity) {
    ...
    calculateSweptExtents: // Used for kinematic and dynamics.
    function collisionObjectSweptExtentsFn(extents) {
    ...
    calculateExtents: // use for all types.
    function collisionObjectCalculateExtentsFn(extents) {
        var shape = this.shape;
    ...
    rayTest: // used for all types.
    function collisionObjectRayTestFn(ray) {
    ...
    integratePositionWithVelocities: // used for kinematics and dynamics
    function intPosWithVelFn(transform, outTransform, timeStep, offset) {
    ...
    applyBiasVelocities: // used for dynamics.
    function applyBiasVelocitiesFn(timeStep) {
        var velocity = this.velocity;
    ...
    integratePosition: // used for kinematics and dynamics.
    function integratePositionFn(timeStep) {
        this.integratePositionWithVelocities(this.startTransform, this.transform, timeStep, 0);
    },
    refreshInertiaTensor: // used for dynamics.
    function refreshInertiaTensorFn() {
    ...
    integrateVelocity: // used for dynamics.
    function integrateVelocityFn(gravity, timeStep) {
        var velocity = this.velocity;
        var pow = Math.pow;
    ...
    isActiveVelocity: // Return false if body is (considering purely velocity) able to sleep.
    // used for dynamics.
    function isActiveVelocityFn(linear, angular) {
    ...
    isActive: // Return false if body is (taking into account sleep delay) able to sleep.
    // used for dynamics.
    function isActiveFn() {
        /* timeStep */ if(!this.permitSleep) {
            return true;
        }
...
var initPrivateBody = function initPrivateBodyFn(r, params) {
...
WebGLPhysicsCollisionObject.sharedInverseInertia = new Float32Array([
    0, 
    0, 
    0, 
    0, 
...
WebGLPhysicsConstraint.prototype = {
    version: 1,
    preStep: function constraintPreStepFn() {
        /* timeStep */     },
...
WebGLGJKContactSolver.prototype = {
    ...
    function gjkEvaluateFn(cache, xformA, xformB) {
        var axis = cache.axis;
...
WebGLContactEPA.prototype = {
    ...
    //   closestA <-- to be populated by this function
    //   closestB <-- to be populated by this function
    function epaEvaluateFn(gjkSimplex, cache, xformA, xformB) {
        var shapeA = cache.shapeA;
        var shapeB = cache.shapeB;
        var hull = this.hull;
...
WebGLPhysicsContact.publicContacts = [
...
WebGLPrivatePhysicsWorld.prototype = {
    ...
    trianglePlaneDiscard: // Determine if shape intersects the plane containing triangle number 'index' in triangle array
    // With given shape and triangle transforms.
    function trianglePlaneDiscardFn(shape, xform, triangleArray, index, txform) {
    ...
    filtered: // Determine if pair of objects is permitted to collide.
    function filteredFn(objectA, objectB) {
        if(objectA === objectB) {
            return true;
        }
    ...
    narrowPhase: // perform narrow phase collision detection between shapes A and B
    // owned by respective objects objectA, objectB (objectA.shape === shapeA etc)
    function narrowPhaseFn(shapeA, shapeB, objectA, objectB) {
    ...
    computeSleeping: // Compute islands of interaction rigid bodies and constraints
    // And put to sleep those islands that are to be considered
    // stationary.
    function computeSleepingFn(timeStep) {
    ...
    wakeIsland: // Wake up a sleeping island.
    function wakeIslandFn(island) {
        while(island.bodies.length > 0) {
    ...
    wakeBody: // Wake up a rigid body.
    function wakeBodyFn(body) {
    ...
    syncBody: // Sync body with broadphase
    function syncBodyFn(body) {
    ...
    wakeConstraint: // Wake up a constraint
    function wakeConstraintFn(constraint) {
        constraint.wakeTimeStamp = this.timeStamp + (this.midStep ? 0 : 1);
    ...
    dynamicSweep: // Implemenmtation of Conservative Advancement for two moving objects.
    function dynamicSweepFn(toi, timeStep, lowerBound, negRadius) {
    ...
    seperatingTOI: // Determine if TOI event corresponds to a seperation of the objects, and can be ignored.
    function seperatingTOIFn(toi) {
        var objectA = toi.objectA;
        var objectB = toi.objectB;
    ...
    staticSweep: // Implemenmtation of Conservative Advancement for a moving body against a static body.
    // (Optimised compared with dynamicSweep)
    function staticSweepFn(toi, timeStep, lowerBound, negRadius) {
    ...
    rayTest: function worldRayTestFn(ray) {
        ...
        var ret = AABBTree.rayTest([
            this.staticSpatialMap, 
            this.dynamicSpatialMap
        ], pRay, rayCallback);
    ...
    //   closestA <-- to be populated by this function
    //   closestB <-- to be populated by this function
    function contactPairTest(cache, xformA, xformB) {
        var axis = cache.axis;
        var shapeA = cache.shapeA;
        var shapeB = cache.shapeB;
        var supportA = cache.closestA;
    ...
    convexSweepTest: // callback of the form HitResult -> Bool
    // if callback is undefined, then a callback of function (x) { return true; } is implied.
    function convexSweepTestFn(params, callback) {
        //
...
var WebGLPhysicsDevice = (function () {
    ...
    WebGLPhysicsDevice.create = function create() {
        /* params */ return new WebGLPhysicsDevice();
    };
